...
- **The home page is a still frame.** With JavaScript, a mouse or a trackpad (`pointer:
  fine`) and a window at least 480px tall, the home page holds still and scrolling moves
  the reader through its five rows (`src/scripts/still-frame.ts`). The row on screen fades out rising 16px, the frame is
  ...
  so a link opens on its row and Back still leaves the page. Below 1024px (a narrow
  window, with a mouse) the response is a drawer along the foot of the frame. With
  reduced motion the rows swap at the midpoint with no rise. Each row arrives at rest,
  which is how the rule above still holds. The arrow in *1 of 5 · Work next* nudges four
  times and stops, and not at all once the reader has scrolled: a hint that never gives
  up is a nag. On a touch screen, or without JavaScript, the home page is the ordinary
  scrolling reference: everything the frame moves is set from scroll events, and a
  phone delivers those after it has already scrolled, so the frame stutters under a
  finger (the 2026-09-16 row in §9). Home page only: a write-up is a long read and would
  spend its life scrolling inside the frame.
...
| Page | Rows | Chrome |
|---|---|---|
| `/` | `GET /api/dylan` (the name, the two-register nameplate sentence, the facts list, See the work / Resume / About me) → `GET /api/work` (one at work, one for myself, one on the side, with an excerpt of a real hook beside the console) → `GET /api/experience` (the three jobs) → `GET /api/away-from-work` (the first program, *Play here* in the side panel; the help desk) → `POST /api/contact` (what I'm looking for; the console sends). Person first, then the work, then the path, then the rest of life, then how to reach me. With JavaScript and a mouse the five rows are one still frame (§3.5), with where-you-are (dots, *2 of 5 · About next*) at the foot of the prose column and, in a window under 1024px, the response drawer; on a phone the page scrolls. | Full |
...
| The still frame needs a fine pointer | Every visible motion in the frame is set from scroll events, and iOS delivers those to the page after the compositor has already scrolled, at its own cadence, so on a phone the prose moves a beat behind the finger. The handler's own cost is under 1ms a frame; the lag is the engine's. A touch screen gets the ordinary scrolling page. | `still-frame.ts` |
