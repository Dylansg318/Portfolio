...
- **The home page is a still frame.** With JavaScript, and a window at least 480px tall,
  ...
  once the reader has scrolled: a hint that never gives up is a nag. **The compositor
  moves the rows, not the script.** Where the browser has scroll-driven animations
  (`.still-css`), the script generates one `@keyframes` per prose column and per response
  (fade in, hold, scroll, hold, fade out, in px) and writes the row's whole life as its
  `animation-range`; nothing depends on a finished animation holding its last frame, on
  two animations stacking, or on a `var()` in a keyframe. It sets opacity and
  transform itself only where it must — no scroll timelines, or reduced motion — and on
  a touch screen that fallback is not offered at all, because a phone delivers scroll
  events after it has already scrolled and anything moved from them lags the finger
  (the 2026-09-16 rows in §9). The settle-to-row waits for the finger to lift. Without
  JavaScript the home page is the ordinary scrolling reference. Home page only: a
  write-up is a long read and would spend its life scrolling inside the frame.
...
| The still frame's rows move on the compositor | iOS delivers scroll events to the page after the compositor has already scrolled, at its own cadence, so anything moved from them lags the finger; the handler's own cost is under 1ms a frame, the lag is the engine's. The script keeps the model and generates one keyframes block and one range per column of each row; CSS scroll-driven animations (Safari 26.4 and Chrome run them on the compositor thread) do the motion. One animation per element, both ends at the rest state: the phone's compositor runs in its own process and dropped a finished animation's fill, which a stacked in/scroll/out design depended on. The script's own painting survives as the fallback — for a mouse, or reduced motion — never for a finger. `inert` lands a beat late on that path and is hit-through, so the JS path's `visibility: hidden` is not used there. | `still-frame.ts`, `global.css` |
...
- If it adds copy: both registers, plain as the fallback, no slogans, numbers dated, and
  in Dylan's language — the 2026-09-10 rules in §9, on every page, new or edited. Site
  voice ("the prose says it in words, the console shows the exact shape") is the drift.
...
| 2026-09-16 | **The still frame is for a mouse; a phone gets the page.** Dylan's iPhone 16 Pro Max stuttered scrolling. Measured in WebKit as that phone (440x763, 3x): the frame's handler costs under 1ms a frame, every other page scrolls at a clean 60Hz with no handler at all, and the home page is the one page whose visible motion is set from JavaScript on scroll — on a phone most of the scroll is the row's prose being moved inside the frame by a transform, a fake scroll. iOS hands scroll events to the page after the compositor has already scrolled, at its own cadence, so that motion lags the finger and stutters; no amount of trimming the handler fixes delivery. The frame now needs `(pointer: fine)` as well as 480px of height; a touch screen gets the ordinary scrolling reference, the layout the page already had without JavaScript. The response drawer survives for a narrow window with a mouse. Not done, Dylan's call: keeping the frame on phones by driving the row motion with CSS scroll-driven animations (`animation-timeline: scroll()`), which Safari 26.4 runs on the compositor thread — about a session, with the settle and the address still in JavaScript, and a touch-specific rule for settling only after the finger lifts. Rule: a scroll-linked effect written in JavaScript is a desktop effect; on a phone, move it to the compositor or don't move it. |
| 2026-09-16 | **The frame comes back to phones, on the compositor.** Dylan's call, the same day. The model stays in the script — segments, settle, jumps, address, rail, drawer — and only the motion moves: `measure()` writes each row's three ranges (in, scroll, out) as absolute scroll offsets in `animation-range`, its overflow in `--ov`, and three keyframe animations on the document's scroll timeline do what `render()` did, in list order with `fill-mode: forwards` and replace composition so the later range wins while it runs and the earlier one's end holds between. The first row has no in, the last no out, a row that fits no scroll. Tested before building (WebKit 2336 as the iPhone, and Chromium): px ranges are scroll offsets, a changed `--ov` or range applies live, `none` in a slot drops that animation. Proved after: 65 scroll offsets × five rows × prose and response, the CSS path within 0.02 opacity and 3px of the JS path on both engines; the handler still under 1ms a frame. Rejected: `visibility` in the keyframes — WebKit accelerates an animation only when every property in it can be, and a main-thread flip would pop the fade-in; the script's `inert`, a beat late and hit-through, is enough. Also: the settle waits for `touchend`, since a page that moves under a finger holding still is a page that fights. Rule: a scroll-linked effect is written for the compositor first; the script may keep the model, never the motion, on a phone. |
| 2026-09-16 | **Work and About went blank on the phone.** The first compositor cut stacked three animations per column — in, scroll, out — with `fill-mode: forwards` holding the row visible between its fade in and its fade out, over a rest state of opacity 0. Desktop WebKit and Chromium honour that (65-offset parity, four devices, zero mismatches); the iPhone did not — its compositor runs in another process and let a finished animation go, so a row was visible only while fading, blank in between, "blacks out for a bit". Not reproducible here; reasoned from the symptom (the Home row, the one with no fade in, was fine) and designed out rather than patched: one animation per column, its `@keyframes` generated by the script for that row with every value in px, its range the row's whole life, both ends the rest state, the first row's range from the top of the document and the last row's past the end — so a compositor that drops a finished or unstarted animation shows exactly what one that holds it shows. No `var()` in a keyframe, no `none` slots. Proved the same way as before, plus a check that every generated animation's ends equal the rest state or its range cannot be left. Rule: on a compositor you don't control, never depend on fill, on stacking, or on a variable resolving — write the whole motion into one animation whose ends are the rest state. |
...
| 2026-09-16 | The still frame needs a fine pointer: §3.5 the still-frame rule, §4's `/` row, one §7 row, one §9 row. |
| 2026-09-16 | The frame back on phones, on the compositor: §3.5 the still-frame rule rewritten, §4's `/` row, the §7 row replaced, one §9 row. |
| 2026-09-16 | One animation per column after the phone dropped the fill: §3.5 one sentence, the §7 row, one §9 row. |
