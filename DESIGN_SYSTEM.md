...
- **The home page is a still frame.** With JavaScript, and a window at least 480px tall,
  ...
  (`.still-css`), the script writes each row's fade-in, scroll and fade-out as scroll
  offsets in `animation-range` and the keyframes do the rest; it sets opacity and
...
| The still frame's rows move on the compositor | iOS delivers scroll events to the page after the compositor has already scrolled, at its own cadence, so anything moved from them lags the finger; the handler's own cost is under 1ms a frame, the lag is the engine's. The script keeps the model and writes each row's ranges; CSS scroll-driven animations (Safari 26.4 and Chrome run them on the compositor thread) do the motion. The script's own painting survives as the fallback — for a mouse, or reduced motion — never for a finger. `inert` lands a beat late on that path and is hit-through, so the JS path's `visibility: hidden` is not used there. | `still-frame.ts`, `global.css` |
